...
#import "utils.typ": bisection_method, calc_median

= Задание 1. Классическая минимизация функции одной переменной. <subtask-1>

Найти аналитическое решение задачи $min f(x), x in [a, b]$ с различной точностью $(epsilon_1=0,1; epsilon_2=0,001)$

== Функция $f(x) = (x-2)^2 -> min, x in [0, 3]$
...
== Функция $f(x) = sin 2pi x - 2pi e^(x-1) -> min, x in [0.5, 1.5]$
...
#figure(
  ...
  table(columns: (auto, auto, auto, auto, auto, auto, auto, auto, auto), table.header(
    ...
    [#calc.round(calc_median(0.5, 1.5) - epsilon_1, digits: 3)],
    [#calc.round(calc_median(0.5, 1.5) + epsilon_1, digits: 3)],
    ...
    [#calc.round(calc_median(1, 1.5) - epsilon_1, digits: 3)],
    [#calc.round(calc_median(1, 1.5) + epsilon_1, digits: 3)],
    ...
    [#calc.round(calc_median(1.375, 1.5) - epsilon_1, digits: 3)],
    [#calc.round(calc_median(1.375, 1.5) + epsilon_1, digits: 3)],
...
#figure(
  ...
  table(columns: (auto, auto, auto, auto, auto, auto, auto, auto, auto), table.header(
    ...
    [1.5],
    [#calc.round((1.5 - 1.375) / 2, digits: 3)],
    [#calc.round(calc_median(1.375, 1.5) - epsilon_2, digits: 3)],
    [#calc.round(calc_median(1.375, 1.5) + epsilon_2, digits: 3)],
    ...
    [#calc.round(calc_median(1.438, 1.5) - epsilon_2, digits: 3)],
    [#calc.round(calc_median(1.438, 1.5) + epsilon_2, digits: 3)],
    ...
    [#calc.round(calc_median(1.469, 1.5) - epsilon_2, digits: 3)],
    [#calc.round(calc_median(1.469, 1.5) + epsilon_2, digits: 3)],
    ...
    [#calc.round(calc_median(1.485, 1.5) - epsilon_2, digits: 3)],
    [#calc.round(calc_median(1.485, 1.5) + epsilon_2, digits: 3)],
    ...
    [#calc.round(calc_median(1.493, 1.5) - epsilon_2, digits: 3)],
    [#calc.round(calc_median(1.493, 1.5) + epsilon_2, digits: 3)],
    ...
    [#calc.round(calc_median(1.497, 1.5) - epsilon_2, digits: 3)],
    [#calc.round(calc_median(1.497, 1.5) + epsilon_2, digits: 3)],
    [#calc.round(func_2(1.498), digits: 3)],
...
#figure(caption: [Метод деления отрезка пополам на языке `typst`@typst-web], ```typ
// Определение функции
#let func_2(x) = {
  calc.sin(2 * calc.pi * x) - 2 * calc.pi * calc.exp(x - 1)
}
...
#figure(caption: [Сравнительная талблица методов нахождения минимума функции], table(
...
В ходе выполнения задания была проведена классическая минимизация функции одной переменной.
Для функции $f(x) = (x-2)^2$ минимум был найден аналитически с использованием производной. Получено точное решение $x_min=2$, при котором значение функции равно $f(x_min) = 0$

Для функции $f(x) = sin 2pi x - 2pi e^(x-1)$ аналитическое решение получить невозможно из-за трансцендентного характера функции, поэтому был применён численный метод деления отрезка пополам.

При уменьшении значения точности ε количество итераций увеличивается, однако результат приближается к более точному значению минимума функции. Сравнение с результатами библиотеки `scipy` показало хорошее совпадение вычислений, что подтверждает корректность проведённых расчетов.
